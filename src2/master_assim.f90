...
!
 do i=1,nlon
   ...
   u5_i(i) = -16.0*cos(2.0*pi*(float(i)-1.0)/float(nlon))
 enddo 
! 
...
!
 ...
 call burgers(xint,xoutt,dt,npdt)
!
...
! Change of variable 
 ...
 do k=1,3  ! start iteration for conjugate gradient 
...
! 2) Term b
   ...
   if (k == 1) then ! needs to be estimated only once - unchanged during minimisation
     d0s(:) = d0(:,1)/sigmao**2
...
! Define the step in the descent direction
  ...
  call hopL_tl(xin5,dkm,yo5,yo,sigmab,c1,dt,npdt,islot) 
  ...
  call hopL_ad(xin5,adk,yo5,yo,sigmab,c1,dt,npdt,islot)
   ...
   zgr1 = 0.0
   do m=-mm,mm
     zsum = zsum + conjg(dkm(m))*(dkm(m) + adk(m))
     zgr1 = zgr1 + conjg(gradientm(m))*gradientm(m) 
   enddo  
   alphak =  zgr1/zsum
   
   print *,'alphak=',alphak,'norm gradient',zgr1,' iteration=',k
 
! Define a new value of the state vector 
 
   chi(:) = chi(:) + alphak*dkm(:)
 
! Swapp gradients and directions
...
!
 call burgers(xin6,xout6,dt,npdt)
!
! Back in physical space
! 
 ...
 do i=1,nlon
   write (60,*) i,u5_f(i),ut_f(i),u6_f(i)
 enddo 
